...
int main() {
    ...
    a.add_card(" A data type that can typically be handled directly by the underlying hardware","Primitive type","Sharing from grandma to ur dad");
    a.add_card("A data type consisting of a fixed set of constant values called enumerators","Enumerated type","Has ENUMS!");
    a.add_card("A template encapsulating data and code that manipulates it","Class","Remeber cookie cutter");
    ...
    std::string response=_rand.attempt(ans[stoi(input)]);
